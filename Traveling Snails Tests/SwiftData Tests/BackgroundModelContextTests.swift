...
struct BackgroundModelContextTests {
    ...
    @Test("Background context manager should be created from main container")
    func testBackgroundContextManagerCreation() async {
        ...
        let result = await backgroundManager.performBackgroundSave(context: "Test creation") { backgroundContext in
    ...
    @Test("Background save should not block main thread")
    ...
    @Test("Background context should maintain data consistency with main context")
    ...
    @Test("Background context should handle errors gracefully")
    func testBackgroundContextErrorHandling() async {
        ...
        let saveResult = await backgroundManager.performBackgroundSave { backgroundContext in
            // Create trip with invalid data that should cause save to fail
            let trip = Trip(name: "", notes: "")  // Empty name might cause validation error
        ...
        switch saveResult {
        case .success:
            Issue.record("Expected save to fail with invalid data")
        case .failure:
            // This is expected - invalid data should cause failure
            break
        }
    }
    
    @Test("Background context should properly cleanup resources")
    ...
    @Test("Background context should work with EditTripView save operations")
