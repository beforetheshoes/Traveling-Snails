...
/// Attachment picker that handles both photos and documents with proper cleanup
struct AttachmentPickerView: View {
    private enum RequestedAction: Equatable {
        case photoButtonTap
        case processSelectedPhoto
        case processSelectedDocument
    }
    ...
    @State private var store: StoreOf<AttachmentPickerFeature>

    // State
    @State private var selectedPhotoItem: PhotosPickerItem?
    @State private var requestedAction: RequestedAction?
    @State private var queuedPhotoItem: PhotosPickerItem?
    @State private var queuedDocumentURL: URL?
    ...
    var body: some View {
        ...
        Menu {
            if allowsPhotos {
                Button {
                    ...
                    requestedAction = .photoButtonTap
                } label: {
            ...
            if allowsDocuments {
                Button {
                    store.showingDocumentPicker = true
                } label: {
        ...
        .onChange(of: selectedPhotoItem) { _, newItem in
            if let newItem = newItem {
                queuedPhotoItem = newItem
                requestedAction = .processSelectedPhoto
            }
        }
        .fileImporter(
            isPresented: $store.showingDocumentPicker,
        ...
        .permissionEducationAlert(
            isPresented: $store.showingPermissionAlert,
    ...
    private func handlePhotoButtonTap() async {
        ...
        case .granted, .limited:
            ...
            store.showingPhotosPicker = true

        case .denied:
            // Permission denied, show education alert
            store.showingPermissionAlert = true
    ...
    private func handleDocumentSelection(_ result: Result<[URL], Error>) {
        ...
        case .success(let urls):
            ...
            queuedDocumentURL = url
            requestedAction = .processSelectedDocument
    ...
    private func processFile(url: URL, originalName: String) async throws {
        ...
        #if DEBUG
        Logger.shared.debug("File attachment prepared successfully", category: .filePicker)
        #endif
    }
    ...
    private func handleRequestedAction() async {
        ...
        case .processSelectedPhoto:
            guard let item = queuedPhotoItem else { return }
            queuedPhotoItem = nil
            await handlePhotoSelection(item)
        case .processSelectedDocument:
            guard let url = queuedDocumentURL else { return }
            queuedDocumentURL = nil
...
extension AttachmentPickerView {
    /// Photo-only picker
    static func photos(onSelected: @escaping (EmbeddedFileAttachment) -> Void, onError: ((String) -> Void)? = nil) -> AttachmentPickerView {
    ...
    static func documents(onSelected: @escaping (EmbeddedFileAttachment) -> Void, onError: ((String) -> Void)? = nil) -> AttachmentPickerView {
        AttachmentPickerView(
            allowsPhotos: false,
    ...
    /// All files picker
    static func allFiles(onSelected: @escaping (EmbeddedFileAttachment) -> Void, onError: ((String) -> Void)? = nil) -> AttachmentPickerView {
        AttachmentPickerView(
            allowsPhotos: true,
            allowsDocuments: true,
            onFileSelected: onSelected,
