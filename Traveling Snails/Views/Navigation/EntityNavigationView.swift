...
struct EnhancedItemRowView<Item: NavigationItem>: View {
    let item: Item
    let isSelected: Bool
    private var showProtectedBadge: Bool {
        guard let trip = item as? Trip else { return false }
        return trip.isProtected
    }
